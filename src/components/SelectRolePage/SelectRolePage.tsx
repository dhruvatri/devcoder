...
const SelectRolePage = () => {
      ...
      if (userSnapshot.exists()) {
        ...
        if (userData.role) {
          if (userData.role === 'Student') {
            ...
            setModalMessage('Thanks for registering as Creator. Please wait for admin approval.');
            setShowModal(true);
          }
        }
      }
  ...
  const handleRoleSelection = async (role: string) => {
    ...
    if (user) {
      ...
      if (role === 'Student') {
        setModalMessage('Successfully registered as Student! Redirecting to login...');
  ...
  return (
    <div className="select-role-page">
      ...
      {showModal && (
        ...
        >
          <motion.div
            className={`modal-content ${selectedRole === 'Student' ? 'success' : 'warning'}`}
